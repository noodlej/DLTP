...
class WaterbirdsDataset(torch.utils.data.Dataset):
    def __init__(self, metadata_path, root_dir, split='train', transform=None):
        df = pd.read_csv(metadata_path)

        split_map = {'train': 0, 'val': 1, 'test': 2}
        split_code = split_map[split]

        self.data = df[df['split'] == split_code].copy()
        self.data['img_filename'] = self.data['img_filename'].apply(
            lambda x: os.path.join(root_dir, x)
        )
        self.targets = self.data['y'].values.astype(int)
    ...
    def __getitem__(self, idx):
        ...
        image = Image.open(row['img_filename']).convert('RGB')
        label = int(row['y'])
        if self.transform:
...
def load_data_subset(batch_size,
    ...
    if dataset == 'cifar10':
        ...
        metadata_path = os.path.join(data_target_dir, 'data/waterbirds_v1.0/metadata.csv')
        img_root = os.path.join(data_target_dir, 'data/waterbirds_v1.0')
        train_data = WaterbirdsDataset(metadata_path, img_root, split='train', transform=train_transform)
        test_data = WaterbirdsDataset(metadata_path, img_root, split='test', transform=test_transform)
        num_classes = 2
    else:
