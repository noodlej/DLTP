...
cost_matrix_dict = {
    ...
    '14': cost_matrix(14, device).unsqueeze(0),
...
def mixup_process(out,
    ...
    if graph:
        save_dir = f'synthetic_images'
        os.makedirs(save_dir, exist_ok=True)

        out_for_save = out.clone().detach().cpu()

        out_for_save = denormalize(out_for_save, mean, std)
        out_for_save = torch.clamp(out_for_save, 0, 1)

        for i in range(out_for_save.size(0)):
            vutils.save_image(out_for_save[i], os.path.join(save_dir, f'synth_{i}.png'))
...
def mixup_graph(input1,
    ...
    #디버깅
    print("DEBUG -- mask min/max:", mask.min().item(), mask.max().item())
    print("DEBUG -- input1 min/max:", input1.min().item(), input1.max().item())
    print("DEBUG -- input2 min/max:", input2.min().item(), input2.max().item())
