...
def make_args_list(n_trials, dataset_names, algorithms, n_hparams_from, n_hparams, steps,
    data_dir, task, holdout_fraction, single_test_envs, test_envs, hparams, args):
    args_list = []
    for trial_seed in range(n_trials):
        for dataset in dataset_names:
            for algorithm in algorithms:
                #NOTE: modified with new arg "test_envs" to incorporate more efficient experimentation, especially for coloredMNIST,
                # whose split [2] is the only split that makes sense to test on
                ...
                for test_env in all_test_envs:
                    # NOTE: Modified by adding n_hparams_from+ in front of n_hparams
                    for hparams_seed in range(n_hparams_from, n_hparams_from+n_hparams):
                        train_args = {}
                        ...
                        if hparams is not None:
                            ...
                            # train_args['hparams'] = json.dumps(json.loads(train_args['hparams']))
                        if args.save_model:
                            train_args['save_model'] = True
                        if args.log_train_env:
                            train_args['log_train_env'] = True
...
if __name__ == "__main__":
    ...
    parser.add_argument('command', choices=['launch', 'delete_incomplete', 'delete_all', 'relaunch'])
    ...
    parser.add_argument('--log_train_env', action='store_true')
    ...
    if args.command == 'launch':
        ...
        to_relaunch = [j for j in jobs if j.state == Job.INCOMPLETE or j.state == Job.NOT_LAUNCHED]
        print(f'About to relaunch {len(to_relaunch)} jobs.')
        if not args.skip_confirmation:
            ask_for_confirmation()
        Job.delete(to_relaunch)
        launcher_fn = command_launchers.REGISTRY[args.command_launcher]
        Job.launch(to_relaunch, launcher_fn)
