...
# --- AIDA 어댑터 등록 (WILDSWaterbirdsBG, WILDSWaterbirds) ---
try:
    import os, importlib.util
    adapter_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..', 'aida', 'datasets.py'))
...
if __name__ == "__main__":
    ...
    # Track best worst-group val accuracy
    ...
    for step in range(start_step, n_steps):
        ...
        if (step % checkpoint_freq == 0) or (step == n_steps - 1):
            ...
            if args.save_model_every_checkpoint:
                # Only save checkpoint if worst-group validation accuracy improved
                # Compute worst-group val acc from results
                val_accs_list = []
                for key, val in results.items():
                    if key.startswith('val') and key.endswith('_acc'):
                        val_accs_list.append(val)
                if val_accs_list:
                    curr_worst = min(val_accs_list)
                    if curr_worst > best_worst_group_acc:
                        best_worst_group_acc = curr_worst
                        save_checkpoint('model_best.pkl')
