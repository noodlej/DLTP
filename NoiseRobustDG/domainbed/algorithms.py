...
class AbstractDANN(Algorithm):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        # by default this penalty is 0, but not sure why they included this here. 
...
class GroupDRO(ERM):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        for m in range(len(minibatches)):
            x, y = minibatches[m]
            # losses[m] = self.compute_loss_on_input(x, y)
            losses[m] = F.cross_entropy(self.predict(x), y)
            # it doesn't seem to have the adjusted loss implemented here for GroupDRO
...
class Fishr(Algorithm):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        # modification, compute fishr penalty on detached features
        detached_z = all_z.detach()
        fishr_logits = self.classifier(detached_z)
        penalty = self.compute_fishr_penalty(fishr_logits, all_y, len_minibatches)
...
class FocalERM(ERM):
    """Simplified V-REx algorithm"""
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(FocalERM, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        self.register_buffer('update_count', torch.tensor([0]))
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        all_logits_idx = 0
        losses = torch.zeros(len(minibatches))
        loss_coeff = torch.zeros(len(minibatches))
        for i, (x, y) in enumerate(minibatches):
            logits = all_logits[all_logits_idx:all_logits_idx + x.shape[0]]
            all_logits_idx += x.shape[0]
            nll = F.cross_entropy(logits, y)
            losses[i] = nll
            loss_coeff[i] = nll.detach()
        loss_coeff = 1+ self.hparams["vrex_lambda"]*2*(loss_coeff - loss_coeff.mean())

        if self.update_count >= self.hparams["vrex_penalty_anneal_iters"]:
            loss = losses*loss_coeff
        else:
            loss = losses
        loss = loss.mean()
...
class SIRM(ERM):
    """Simple Invariant Risk Minimization. Only have one environment
    This do not work as well as IRM, meaning that at least 2 environments are needed.
    However, this approach does exhibit some level of noise robustness.
    """

    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(SIRM, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        self.register_buffer('update_count', torch.tensor([0]))
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        penalty = self._irm_penalty(all_logits, all_y)
        loss = nll + (penalty_weight * penalty)
...
class ERMReg(ERM):
    """
    ERM with heavy regularization. Settings in hparams_registry.py
    """

    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(ERMReg, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
